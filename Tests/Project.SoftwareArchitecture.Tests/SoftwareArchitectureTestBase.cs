...
{
    public abstract class SoftwareArchitectureTestBase : AbpIntegratedTestBase<SoftwareArchitectureTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected SoftwareArchitectureTestBase()
        {
        ...
        protected void UsingDbContext(Action<SoftwareArchitectureDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SoftwareArchitectureDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SoftwareArchitectureDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SoftwareArchitectureDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SoftwareArchitectureDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SoftwareArchitectureDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SoftwareArchitectureDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SoftwareArchitectureDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SoftwareArchitectureDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SoftwareArchitectureDbContext, Task<T>> func)
        {
            T result;
